...
def profile(update: Update, context: CallbackContext):
    ...
    if not ig_link:
        ...
        update.message.reply_text(f"✅ IG Saved: `{new_ig}`\n\n🎁 **Welcome Bonus: +300 Coins!**", parse_mode='Markdown')
    else:
        db("UPDATE users SET ig_link=? WHERE user_id=?",(new_ig, uid))
        update.message.reply_text(f"✅ IG Updated: `{new_ig}`", parse_mode='Markdown')
...
def earncoin(update: Update, context: CallbackContext):
    ...
    if not order: update.message.reply_text("❌ No orders available right now."); return
...
def order(update: Update, context: CallbackContext):
    ...
    if amount < 100 or amount > 1000000: update.message.reply_text("❌ Min 100, Max 1,000,000"); return
    cost = int((amount / 1000) * PRICE_1000_FOLLOWERS)
    coins = get_user(uid)[1]
    if coins < cost: update.message.reply_text(f"❌ You need {cost} Coins. You have `{coins}`"); return
...
def addbal(update: Update, context: CallbackContext):
    ...
    if not qr or not qr[0][0]: update.message.reply_text("❌ Admin has not set QR yet"); return
...
def redeem(update: Update, context: CallbackContext):
    uid = update.effective_user.id
    if not context.args: update.message.reply_text("Usage: `/redeem CODE123`"); return
    code = context.args[0]
...
def setqr(update: Update, context: CallbackContext):
    ...
    update.message.reply_text("✅ QR Set Successfully")
...
def button(update: Update, context: CallbackContext):
    ...
    elif q.data == "earn": earncoin(q.message, context)
    elif q.data == "order": q.message.reply_text("Use: `/order <ig_link> <amount>`", parse_mode='Markdown')
    elif q.data == "profile": profile(q.message, context)
    elif q.data == "community": community(q.message, context)
    elif q.data.startswith("buy_"):
        amount = int(q.data.split("_")[1])
        price = {10000:30, 20000:55, 100000:150, 1000000:300}[amount]
        bal = get_user(uid)[2]
        if bal < price: q.edit_message_text("❌ Low balance. Use `/addbal`"); return
        db("UPDATE users SET balance = balance -?, coins = coins +? WHERE user_id=?",(price, amount, uid))
        q.edit_message_text(f"✅ Bought {amount} Coins! -{price}₹")

def main():
    init_db()
    updater = Updater(TOKEN, use_context=True) # V13 WALA
    dp = updater.dispatcher
    dp.add_handler(CommandHandler("start", start))
    dp.add_handler(CommandHandler("help", help_cmd))
    dp.add_handler(CommandHandler("profile", profile))
    dp.add_handler(CommandHandler("earncoin", earncoin))
    dp.add_handler(CommandHandler("order", order))
    dp.add_handler(CommandHandler("growing", growing))
    ...
    print("✅ IG BOOSTER BOT v1.4 RUNNING ON V13.15")
